...
if args.years is None:
    ...
    fig,axs = plt.subplots(ncols=cwrap,nrows=nrows)
...
for s,season in enumerate(seasons):
    ...
    for a,ax in enumerate(axs.flat):
        if args.years is None:
            ...
            if s == a:
                ...
                continue
        cf = tmpp.plot.contourf(ax=ax,x='lat',levels=levels,extend='both',**transf)
        if not ax.get_subplotspec().is_last_row():
            ax.set_xlabel('')
        if not ax.get_subplotspec().is_first_col():
            ax.set_ylabel('')
        ax.set_title('{0}, {1}{2}'.format(args.var,season,attle))
        ax.grid()
        ac.LogPlot(ax)
    ...
    if s == len(seasons)-1 or args.years is None:
        ...
        outFile = 'figures/{2}_{0}_{1}.pdf'.format(args.var,suff,model)
        if args.qbo:
