...
levels = {'ipv':[l for l in np.linspace(-2e-7,2e-7,21) if l !=0]}
...
if args.years is not None:
    ...
    fig,axs,transf = ac.Projection('PlateCarree',ncols=4,kw_args={'central_longitude':155})
    fig.set_figheight(3)
    ...
    for s,seas in enumerate(dipvsm.season):
        ...
        axs[s].coastlines()
        axs[s].set_title(tvar+' & 200hPa WAF, {0}'.format(seas.values))
